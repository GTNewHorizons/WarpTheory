...
public class WarpSwamp extends IWorldTickWarpEvent {
    ...
    public int triggerEvent(int eventAmount, World world, EntityPlayer player) {
        ...
        if (target.getBlock(world) == Blocks.water) {
            ...
            grown = true;
        } else if (target.getBlock(world).getMaterial() == Material.leaves
            ...
            if (world.rand.nextBoolean()
                    && target.getBlock(world)
                            .canSustainPlant(world, target.x, target.y, target.z, ForgeDirection.UP, (IPlantable)
                                    Blocks.sapling)) {
                if (world.rand.nextBoolean()) {
                ...
                if (target.getBlock(world) != Blocks.dirt && target.getBlock(world) != Blocks.grass) {
                    ...
                    else if (world.rand.nextBoolean()) world.setBlock(target.x, target.y, target.z, Blocks.grass, 0, 3);
