    ...
    public int triggerEvent(int eventAmount, World world, EntityPlayer player) {
        BlockCoord target =
                RandomBlockHelper.randomBlock(
                        world, player, 8, block -> MiscHelper.hasNonSolidNeighbor(world, block));
        if (target == null) {
            return 0;
        }
        ...
        if (target.getBlock(world) == Blocks.water) {
            ...
            grown = true;
        } else if (target.getBlock(world).getMaterial() == Material.leaves || target.getBlock(world) == Blocks.log || target.getBlock(world) == Blocks.log2) {
            for (int j = 0; j < 6; j++) {
                int side = 2 + world.rand.nextInt(4);
                if (Blocks.vine.canPlaceBlockOnSide(world, target.x, target.y, target.z, side) && target.offset(side).isAir(world)) {
                    world.setBlock(target.x, target.y, target.z, Blocks.vine, 1 << Direction.facingToDirection[Facing.oppositeSide[side]], 3);
                    grown = true;
                    break;
                }
            }
        } else {
            if (world.rand.nextBoolean() && target.getBlock(world).canSustainPlant(world, target.x, target.y, target.z, ForgeDirection.UP, (IPlantable) Blocks.sapling)) {
                if (world.rand.nextBoolean()) {
                    if (target.offset(1).isAir(world) || (target.getBlock(world) instanceof IPlantable && target.getBlock(world) != Blocks.sapling))
                    ...
                    if (target.offset(1).isAir(world) && target.offset(0).getBlock(world) instanceof IGrowable)
                        ((IGrowable) target.getBlock(world)).func_149853_b(world, world.rand, target.x, target.y, target.z); // Bonemealing
                } else {
                ...
                if (target.copy().offset(1).getBlock(world) == Blocks.log || target.copy().offset(1).getBlock(world) == Blocks.log2)
                    world.setBlock(target.x, target.y, target.z, target.copy().offset(1).getBlock(world), target.copy().offset(1).getMeta(world), 3);
                else
                    world.setBlock(target.x, target.y, target.z, Blocks.water, 0, 3);
                grown = true;
            } else if (WarpHandler.decayMappings.containsKey(new NameMetaPair(target.getBlock(world), target.getMeta(world))) && target.getBlock(world).isOpaqueCube() && target.getBlock(world) != Blocks.log && target.getBlock(world) != Blocks.log2) {
